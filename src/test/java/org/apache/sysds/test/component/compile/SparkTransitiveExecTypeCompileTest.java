...
public class SparkTransitiveExecTypeCompileTest extends CompilerTestBase {
	...
	private static final String TALL_VECTOR_HEADER = "X = rand(rows=5000, cols=200000, seed=1);\n" + // ~8GB -> rand and
																										// rowSums run
																										// on Spark
	...
	public void cumulativeUnaryStaysCP() {
		String dml = TALL_VECTOR_HEADER + "r = cumsum(c);\n" + // sole consumer of the Spark-resident vector, CP by
																// estimate ...
	...
	public void singleConsumerBinaryPulledIntoSpark() {
		String dml = TALL_VECTOR_HEADER + "r = c + 2.0;\n" + // matrix-scalar on the Spark-resident vector, sole
																// consumer -> pulled into Spark
			"print(as.scalar(r[2500,1]));\n";
		Program prog = compile(dml, null, ExecMode.HYBRID, SMALL_MEM_BUDGET);
