...
/**
 * Direct (no DML) round-trip tests for the native Delta Kernel based matrix reader/writer. Each test writes a
 * MatrixBlock to a fresh local Delta table directory and reads it back, asserting dimensions and values match.
 */
public class DeltaMatrixReadWriteTest {
	...
	public void readNonDoubleNumericColumns() throws Exception {
		...
		try {
			...
			writeTypedColumns(tablePath, new DataType[] {LongType.LONG, IntegerType.INTEGER, BooleanType.BOOLEAN},
	...
	public void writerTargetFileSizeConfigProducesMoreFiles() throws Exception {
		...
		try {
			assertEquals("config getter reflects the override", 1L * 1024 * 1024,
	...
	public void factoryRoutesDeltaToParallelWhenEnabled() {
		...
		try {
			...
			assertTrue("expected ReaderDeltaParallel when parallel read enabled", par instanceof ReaderDeltaParallel);
	...
	public void readFloatColumnsCoercedToDouble() throws Exception {
		...
		try {
			...
			writeTypedColumns(tablePath, new DataType[] {FloatType.FLOAT, FloatType.FLOAT}, new double[][] {f0, f1});
	...
	public void readShortByteColumnsCoercedToDouble() throws Exception {
		...
		try {
			...
			writeTypedColumns(tablePath, new DataType[] {ShortType.SHORT, ByteType.BYTE},
	...
	private static void writeTypedColumns(String tablePath, DataType[] types, double[][] vals) throws Exception {
		...
		FilteredColumnarBatch fcb = new FilteredColumnarBatch(new TypedBatch(schema, types, vals, null),
			Optional.empty());
	...
	private static void writeNullableDoubleColumn(String tablePath, double[] vals, boolean[] nulls) throws Exception {
		...
		FilteredColumnarBatch fcb = new FilteredColumnarBatch(
			new TypedBatch(schema, new DataType[] {DoubleType.DOUBLE}, new double[][] {vals}, new boolean[][] {nulls}),
			Optional.empty());
		DeltaKernelUtils.commit(engine, DeltaKernelUtils.qualify(tablePath), schema, singleton(fcb));
	}
	...
	/**
	 * Minimal in-memory columnar batch backed by per-column double[] values, with an optional per-column null mask
	 * ({@code nulls==null} => no nulls).
	 */
	private static class TypedBatch implements ColumnarBatch {
		...
		@Override
		public int getSize() {
			return _vals[0].length;
		}

		@Override
		public ColumnVector getColumnVector(int ordinal) {
			return new TypedVector(_types[ordinal], _vals[ordinal], _nulls == null ? null : _nulls[ordinal]);
		}
	}
	...
	private static class TypedVector implements ColumnVector {
		...
		TypedVector(DataType type, double[] vals, boolean[] nulls) {
			_type = type;
			_vals = vals;
			_nulls = nulls;
		}
		...
		public float getFloat(int rowId) {
			return (float) _vals[rowId];
		}

		@Override
		public long getLong(int rowId) {
			return (long) _vals[rowId];
		}
		...
		public short getShort(int rowId) {
			return (short) _vals[rowId];
		}

		@Override
		public byte getByte(int rowId) {
			return (byte) _vals[rowId];
		}

		@Override
		public boolean getBoolean(int rowId) {
			return _vals[rowId] != 0;
		}
