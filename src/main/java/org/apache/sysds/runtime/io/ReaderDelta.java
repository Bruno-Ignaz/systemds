...
public class ReaderDelta extends MatrixReader {
	...
	/**
	 * Extract one columnar batch into a fresh row-major {@code double[]} of the live (selected, after deletion vector)
	 * rows. No per-cell boxing.
	 */
	...
	/**
	 * Extract one columnar batch directly into a contiguous row-major dense array, placing live (selected, after
	 * deletion vector) rows starting at absolute row {@code destRow}. Avoids the intermediate per-batch buffer and the
	 * subsequent concatenation copy. Returns the number of rows written.
	 */
	static int extractBatchInto(ColumnVector[] cols, int size, boolean[] selected, int[] types, int ncol, double[] dest,
		int destRow) {
		for(int c = 0; c < ncol; c++) {
	...
	static double getDoubleValue(ColumnVector col, int r, int type) {
		switch(type) {
			case DeltaKernelUtils.T_DOUBLE:
				return col.getDouble(r);
			case DeltaKernelUtils.T_FLOAT:
				return col.getFloat(r);
			case DeltaKernelUtils.T_LONG:
				return col.getLong(r);
			case DeltaKernelUtils.T_INT:
				return col.getInt(r);
			case DeltaKernelUtils.T_SHORT:
				return col.getShort(r);
			case DeltaKernelUtils.T_BYTE:
				return col.getByte(r);
			case DeltaKernelUtils.T_BOOLEAN:
				return col.getBoolean(r) ? 1.0 : 0.0;
			default:
				throw new DMLRuntimeException("Unsupported Delta column type code: " + type);
		}
	}

	/**
	 * Resolve a numeric Delta column type to its internal code, rejecting non-numeric columns (e.g. string) that cannot
	 * back a SystemDS matrix.
	 */
	static int numericTypeCode(DataType dt, String name) {
		...
		if(code < 0 || code == DeltaKernelUtils.T_STRING)
			throw new DMLRuntimeException("Unsupported non-numeric Delta column '" + name + "' of type " + dt
				+ " for matrix read (only numeric columns are supported).");
		return code;
	}
